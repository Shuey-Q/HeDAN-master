...
class HeDANLayer(nn.Module):
    ...
    def forward(self, g, h):
        ...
        if self._cached_graph is None or self._cached_graph is not g:
            ...
            for meta_path in self.meta_paths:
                if meta_path in {('follow',), }:
                    ...
                    print('******************interest**********************')
                    self._cached_coalesced_graph[meta_path] = dgl.edge_type_subgraph(g, [('user', 'interest', 'message')])
        ...
        embedings = {}
        follow_g = self._cached_coalesced_graph[('follow',)]
        ntype = "user"
        semantic_embeddings["user"].append(self.hedan_layers[0](follow_g, h[ntype]).flatten(1))
        embedings['user_s'] = semantic_embeddings['user'][0]
        retweet_g = self._cached_coalesced_graph[("retweet",)]
        semantic_embeddings["user"].append(self.hedan_layers[1](retweet_g, embedings['user_s']).flatten(1))
        embedings['user_r'] = semantic_embeddings['user'][1]
        semantic_embeddings["user"] = torch.stack(semantic_embeddings["user"], dim=1)
        embedings["user"] = self.semantic_attention_u(semantic_embeddings["user"])
        interest_g = self._cached_coalesced_graph[("interest",)]
        ntype = "message"
        h_ = (embedings["user"], h['message'])
        semantic_embeddings['message'].append(self.hedan_layers[2](interest_g, h_))
...
class HeDAN(nn.Module):
    def __init__(self, meta_paths, in_size, hidden_size, out_size, aggre_type, num_heads, dropout):
        ...
        self.semantic_attention_batch = SemanticAttention2(in_size= hidden_size * num_heads)
        self.decoder_attention = TransformerBlock(input_size= hidden_size * num_heads, n_heads=2)
    ...
    def forward(self, tgt, tgt_timestamp, tgt_index, g, inputs):
        ...
        batch_h_u_s = F.embedding(tgt, h_trans['user_s'])
        batch_h_u_r = F.embedding(tgt, h_trans['user_r'])

        batch_h = torch.cat((batch_h_m.unsqueeze(1), batch_h_u, batch_h_u_s, batch_h_u_r), dim = 1)
        att_out = self.decoder_attention(batch_h, batch_h, batch_h)
        batch_h_new = att_out.sum(1)
        batch_pre = self.predict_m(batch_h_new)
